...
export function PatientSearchField({
  ...
  useEffect(() => {
    // If value is provided and different from current selection
    if (value && value !== selectedPatient?.id) {
      setIsLoadingPatient(true)

      // Fetch the patient data for the provided ID
      patientService.getById(value)
        .then((patient) => {
          if (patient) {
            // Use the handleSelectPatient to properly set the selected patient
      ...
      // If value is cleared externally, clear the selection
      clearSelection()
    }
  ...
  return (
    <div ref={containerRef} className="relative w-full">
      <div className="relative">
        ...
        <Input
          ...
          onChange={(e) => {
            ...
            // If the input is cleared (empty value), also clear the parent form value
            if (newValue === '') {
              clearSelection()
              onChange?.('')
            }
          ...
          onFocus={() => {
            setIsFocused(true)
            // Don't clear selection on focus - only clear through explicit user actions
          }}
      ...
      {/* Show loading indicator when fetching patient by ID */}
      {isLoadingPatient && (
