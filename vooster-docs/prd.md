...
| **유연한 주치의 지정** | 사전 등록 없이 이름으로 주치의 지정 | Must | ✅ 완료 | 텍스트 입력으로 즉시 지정 가능 |
| | 주치의 등록 시 자동 연결 | Must | ✅ 완료 | 등록된 의사와 자동 매칭 |
| **간소화된 항목 관리** | 3개 카테고리로 단순화 (주사/검사/기타) | Must | ✅ 완료 | injection/test/other 타입으로 구분 |
