...
- ✅ **완료 이력 보존** - 캘린더에서 과거 완료 기록 확인 가능
- ✅ **사용자 승인 시스템** - HIPAA 준수 접근 제어
- ✅ **환자 아카이빙** - 고유 제약 조건 충돌 방지
- ✅ **일정 일시정지/재개** - 유연한 일정 관리
- ✅ **의사 역할 지원** - 의사별 환자 할당 및 관리
...
### 핵심 테이블
- **profiles** - 사용자 프로필 및 승인 워크플로우 (nurse, admin, doctor 역할)
- **patients** - 환자 정보 (아카이빙 지원, 의사 할당)
- **items** - 의료 절차/검사 카탈로그 (injection, test, other 카테고리)
- **schedules** - 반복 의료 일정 정의 (주 단위 간격)
